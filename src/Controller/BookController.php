...
class BookController extends AbstractController
{
    ...
    public function listBooksBefore2023(BookRepository $bookRepository): Response
    {
        $books = $bookRepository->bilelajilibooksbefore2023();

        return $this->render('book/book2023.html.twig', [
            'books' => $books,
        ]);
    }
    ...
    public function updateCategory(BookRepository $bookRepository, ManagerRegistry $managerRegistry)
    {
        ...
        $william= $bookRepository->updateCategoryForWilliam();

        foreach ($william as $book) {
    ...
    public function publishedBooks(BookRepository $bookRepository,Request $req): Response
    {
        ...
        $total = $bookRepository->countCategory();
   ...
   if ($form->isSubmitted()){
    ...
    #[Route('/listBooksDates', name: 'listBooksDates')]
    public function listBooksDates(BookRepository $bookRepository)
    {
        $books = $bookRepository->findBooksDates();

        return $this->render('book/listBooksDates.html.twig', [
            'books' => $books,
        ]);
    }
